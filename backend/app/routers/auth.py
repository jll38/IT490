...
async def onboarding(request: OnboardingRequest):
    ...
    if response.get("success"):
        ...
        raise HTTPException(
            status_code=400, detail="Onboarding failed, please try again later.")
...
async def get_user_settings(username):
    ...
    if response.get('success', False):
        ...
        raise HTTPException(
            status_code=400, detail="Failed Retrieving Settings")
...
async def get_user_settings(request: OnboardingRequest):
    rabbitmq_client = RabbitMQ(queue_name='onboarding_queue')
    response = rabbitmq_client.call(
        {'username': request.username, 'restrictions': request.restrictions, 'tdee': request.tdee, 'method': 'PUT'})
    rabbitmq_client.close_connection()
    print(response)
    if response["success"]:
        return response
